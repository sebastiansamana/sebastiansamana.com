(() => {
    const runtime = (window.__varelismMathVisualisationLabCanvas ||= {
    ...
    const setupHeroCanvas = () => {
      document.querySelectorAll('[data-math-lab-canvas]').forEach((canvas) => {
        if (!(canvas instanceof HTMLCanvasElement) || canvas.dataset.mathLabReady === 'true') return;

        const ctx = canvas.getContext('2d', { alpha: false });
        if (!ctx) return;

        canvas.dataset.mathLabReady = 'true';
        let renderScale = 1;
        let frame = 0;
        let lastPaint = 0;
